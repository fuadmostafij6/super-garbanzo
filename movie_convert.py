...
def _normalize_category(raw_category: Optional[str], title: str) -> str:
    if raw_category and raw_category.strip():
        ...
        if cat.lower() in {"cricket", "football", "soccer", "basketball", "baseball", "hockey", "news", "movies",
                           "kids", "music"}:
...
def parse_m3u(m3u_text: str):
    ...
    for line in lines:
        if line.startswith("#EXTINF"):
            ...
            if title_from_suffix and (
                    "http" in title_from_suffix or "w_300" in title_from_suffix or "w_240" in title_from_suffix):
